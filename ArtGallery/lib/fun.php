<?php
	...
	function getUrl($page){
		...
		$url .= $_SERVER["SCRIPT_NAME"];//http://localhost/artgallary/user.php
		$queryString = $_SERVER["QUERY_STRING"];//page=2&id=1&name=abc
		parse_str($queryString,$queryArr);
		if (isset($queryArr["page"])) {
			unset($queryArr["page"]);
		}
		$queryArr["page"] = $page;
		$queryString = http_build_query($queryArr);//将数组转换为请求字符串
		$url .= "?".$queryString;
		return $url;
	}
	#创建页码函数 用来输出放有页码标签的字符串
