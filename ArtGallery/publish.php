<?php 
	...
	if (!empty($_POST['name'])) {
		...
		$now = time();
		//在入库前对商品进行唯一性验证
		$result = $pdo->query("select count(goods_id) as total from goods where goodsname = '{$name}'");
		...
		//接受上传文件
		$file = $_FILES['file'];
		$pic=imgUpload($file,$userId);//调用fun里面的函数
		// echo $pic;
		$result= $pdo->exec("insert into goods (goodsname,price,des,content,pic,user_id,create_time,update_time,view) values('{$name}','{$price}','{$des}','{$content}','{$pic}','{$userId}','{$now}','{$now}',0)");
		if ($result) {
			msg(1,"操作成功","user.php");
		}else{
			msg(2,"上传失败");
...
<body>
	<div class="header">
			...
			<div class="auth fr">
				<ul>
					<li><a href="#"><span>管理员：<? echo $user;?></span></a></li>
					<li><a href="#">退出</a></li>
				</ul>
			</div>
	</div>
